...
1. **AQI Computation → [Six AQI categories + colors](#six-aqi-categories-colors)** — Hard-code band edges [0,50,100,200,300,400,500] and a 6-color LUT; index = np.searchsorted on the final AQI value for map colorization.
2. **Acquisition / Preprocess → [GEOS-Chem model-ratio scaling (van Donkelaar geophysical method)](#geos-chem-model-ratio-scaling-van-donkelaar-geophysical-method)** — Use MERRA-2/CAMS ratio (not full GEOS-Chem run) to make a gap-free physical PM2.5 prior, then bias-correct/fuse with CPCB via ML. Strong gap-filler for cloud pixels.
3. **ML Prediction → [LSTM / GRU (temporal sequence)](#lstm-gru-temporal-sequence)** — Use as the temporal block inside CNN-LSTM. Standalone only for pure station forecasting, not gridded mapping.
4. **ML Prediction → [Random Forest (RF)](#random-forest-rf)** — Core ensemble member and sanity baseline. Use spatial-block + day-grouped CV; quantile RF for intervals.
...
6. **Fire & Transport → [Biomass-burning period extraction (thresholds + seasonal decomposition)](#biomass-burning-period-extraction-thresholds-seasonal-decomposition)** — Use VIIRS 375m as primary (resolves small field fires), MODIS for 2002+ climatology. Episode = STL-residual > +2 sigma sustained >=2 days; tag cropland vs forest.
7. **Fusion & Gap-fill → [DINEOF (Data-Interpolating Empirical Orthogonal Functions)](#dineof-data-interpolating-empirical-orthogonal-functions)** — FIRST-PASS gap-filler per TROPOMI species cube (NO2/SO2/CO/O3/HCHO) and INSAT AOD before ML; cheap, label-free, gives uncertainty. Pair with ML for nonlinear residuals.
8. **Validation → [Core point metrics: RMSE, R/R2, MAE, MBE](#core-point-metrics-rmse-rr2-mae-mbe)** — MANDATORY headline table: RMSE, MAE, R, R2, MBE overall + by pollutant, season, AQI band.
...
10. **Visualization → [geemap (GEE-native interactive maps)](#geemap-gee-native-interactive-maps)** — USE for EDA, daily-AQI time slider, HCHO seasonal frames, split-map satellite-AQI vs CPCB. Primary notebook viz tool.
